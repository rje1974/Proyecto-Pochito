#ifndef _ROS_hector_mapping_nav_HectorDebugInfo_h
#define _ROS_hector_mapping_nav_HectorDebugInfo_h
...
namespace hector_mapping_nav
{
  ...
  class HectorDebugInfo : public ros::Msg
  {
    public:
      ...
      typedef hector_mapping_nav::HectorIterData _iterData_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(iterData_lengthT > iterData_length)
        this->iterData = (hector_mapping_nav::HectorIterData*)realloc(this->iterData, iterData_lengthT * sizeof(hector_mapping_nav::HectorIterData));
      iterData_length = iterData_lengthT;
      for( uint32_t i = 0; i < iterData_length; i++){
      offset += this->st_iterData.deserialize(inbuffer + offset);
        memcpy( &(this->iterData[i]), &(this->st_iterData), sizeof(hector_mapping_nav::HectorIterData));
      }
     return offset;
    }

    const char * getType(){ return "hector_mapping_nav/HectorDebugInfo"; };
